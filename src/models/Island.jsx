...
import islandScene from "../assets/3d/la_night_city.glb";
export function Island({
  ...
  const { scene, gl, viewport, camera} = useThree();
  const { nodes, materials } = useGLTF(islandScene);
 
  const scaleValue = 2;

  useEffect(() => {
    const hemisphereGeometry = new THREE.SphereGeometry(500, 32, 16, Math.PI, Math.PI * 2, 0, Math.PI / 2);
    const hemisphereMaterial = new THREE.MeshBasicMaterial({ color: 'black', transparent: true, opacity: 0.5 });
    const hemisphereMesh = new THREE.Mesh(hemisphereGeometry, hemisphereMaterial);
    hemisphereMesh.position.y = -250; // Adjust this value as needed
    hemisphereMesh.rotation.x = Math.PI / 2; // Rotate the hemisphere to face up
    hemisphereRef.current = hemisphereMesh;
    scene.add(hemisphereMesh);

    return () => scene.remove(hemisphereMesh);
  }, [scene]);

  useFrame(({ clock }) => {
    const elapsedTime = clock.getElapsedTime();
    if (hemisphereRef.current) {
      hemisphereRef.current.material.opacity = 0.5 + Math.sin(elapsedTime) * 0.25;
    }
  });
  ...
  useFrame(() => {
    ...
    if (!isRotating) {
      ...
      // otating,When r determine the current stage based on island's orientation
  ...
  return (
    ...
    <a.group ref={islandRef} {...props} scale={scaleValue}>
