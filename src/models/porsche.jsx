...
import porsche from '../assets/3d/porsche.glb';
export function Porsche(props) {
  const { isTyping } = props;
  const { nodes, materials } = useGLTF(porsche);
  ...
  useEffect(() => {
    ...
    // This function will be called recursively to animate the wheels
    const animate = () => {
        if (isTyping && wheelRef.current) {
          // Increment the wheel rotation angle around the X-axis
          setWheelRotation((prevRotation) => prevRotation + 0.05);
        }
    ...
    // Cleanup function to stop the animation when the component is unmounted or when typing stops
    return () => {
      ...
      <group scale={0.01}>
        ...
        >
          <mesh
            ...
            rotation={[
                wheelRotation, 
                wheelRef.current ? wheelRef.current.rotation.y : 0, 
                wheelRef.current ? wheelRef.current.rotation.z : 0
              ]}
            castShadow
            receiveShadow
            geometry={
